...
async function loadComponent(path) {
  try {
    ...
    alert('Erro ao carregar a página. Retornando ao painel.');
    return await loadComponent('components/painel.html');
  }
}
...
async function init() {
  ...
  document.addEventListener('click', async (e) => {
    ...
    if (target.matches('#btnAcessar')) {
      ...
      carregarLista();
    } else if (target.matches('#btnVerCredencial')) {
      app.innerHTML = await loadComponent(routes.confirm);
      const { gerarQRCode } = await import('./qrcode-handler.js');
      gerarQRCode('Credencial do Usuário Atual');
      const { animateConfirmCard } = await import('./gsapTransitions.js');
      animateConfirmCard();
    }
  });
